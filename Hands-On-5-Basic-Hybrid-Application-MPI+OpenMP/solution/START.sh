...
plot "file_speedup.data" using 1:2 title "OMP(T=8)"             ls 1 with linespoints,\
     "file_speedup.data" using 1:3 title "MPI(P=6)"             ls 2 with linespoints,\
     "file_speedup.data" using 1:4 title "MPI+OMP(P=4, T=6)"    ls 3 with linespoints
EOF
}
...
printf "\033[1D$i :" 
                      ./sequential       "$i"     >> file1
OMP_NUM_THREADS=8     ./mm-openmp        "$i"     >> file2
      mpirun -np 6    ./mm-mpi           "$i"  2>/dev/null   >> file3
      mpirun -np 4    ./mm-mpi+openmp    "$i"  6 2>/dev/null  >> file4
done
...
rm -f *.txt file* fspeed* *.data mm-mpi mm-openmp mm-mpi+openmp sequential *.plt
echo " "
